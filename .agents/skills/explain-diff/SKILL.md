...
Respond in the user's language, keeping code identifiers unchanged. In OpenAI Codex, invoke this skill as `$explain-diff`; the `/explain-diff` examples below use Claude Code syntax. When the user asks to prepare a spoken explanation or interview answer, finish with a short first-person outline based on the verified changes, results, and limitations.
...
Target (optional, defaults to your working diff):
  <empty>        Branch diff vs upstream/default branch plus local changes;
                 latest commit if there is otherwise nothing to explain
...
- Start with `git status --short` and state the comparison used. Resolve the repository's default branch from `refs/remotes/origin/HEAD`, or an existing `main` / `master`; do not assume that `main` exists.
- **No target**: compare `git diff @{upstream}...HEAD` when an upstream exists, otherwise compare the default branch to HEAD. Also include `git diff HEAD` for tracked local changes and inspect relevant untracked files listed by `git ls-files --others --exclude-standard`. If neither the branch comparison nor local changes contains a diff, explain the latest commit with `git show --format=fuller HEAD --` and disclose this fallback.
- **PR number**: `gh pr view <n>` for the title/description, `gh pr diff <n>` for the diff.
- **Git range**: `git diff <range> --`; read the commit messages with `git log <range>` too.
- **Single commit**: `git show --format=fuller <commit> --` shows the changes introduced by that commit. Do not use `git diff <commit>`: that compares the commit to the working tree and can explain unrelated changes. For a merge commit, state which parent comparison is being explained.
- **Branch**: compare against the repository's default branch with `git diff <default>...<branch> --` and read the corresponding commit messages.
- **Path**: restrict the applicable diff commands to that path, placing it after `--`.

If the selected comparison is empty, say exactly what was compared and stop — do not invent an explanation.
...
**Empty diff** — Report the exact target and comparison. For an implicit target, use the latest-commit fallback from Step 2; for an explicit empty range or path, stop and say no changes were found.
